...
在独立验证集（401 条）上的最终评估结果为：准确率 **85.79%**（344/401）。从 Loss 曲线（下图）来看，前期训练与验证 Loss 同步下降，模型快速收敛；第 5 轮后验证 Loss 从 0.312 回升至 0.384，而训练 Loss 继续下降，呈现典型的过拟合趋势，验证了早停策略的必要性。
...
- **第一层——注意力证据提取**：BERT 最后一层对 [CLS] token 的注意力权重在所有注意力头上取平均，排除 [PAD]、[CLS]、[SEP] 特殊 token 后，筛选 top-10 高分词作为关键证据，直接反映分类决策依据。

- **第二层——RAG 相似案例参照**：SentenceTransformer 将推文编码为 384 维稠密向量，在 FAISS 索引中检索 top-5 语义最相似的训练样本及真实标签，为 LLM 解释提供事实锚点。

- **第三层——LLM 自然语言生成**：将推文原文、预测标签、置信度、关键证据词和参考案例拼接为结构化提示词，调用 qwen3.5-27b 从语言风格、信息来源、事实核查、上下文分析等维度生成连贯中文判断依据。API 不可用时自动降级为模板化解释。该三层设计使系统输出兼具可追溯性与可读性。
